...
module.exports = {
  ...
  run: async function ({ api, event, users, args }) {
    ...
    switch (handleReply.type) {
      case "reply": {
        ...
        if (userReply === correctAnswer.toLowerCase()) {
          ...
          let rewardCoins = 300;
          let rewardExp = 100;
          let users = await users.get(author);
          await users.set(author, {
          money: users.money + rewardCoins,
            exp: users.exp + rewardExp,
            data: users.data,
          });
          api.sendMessage(`Congratulations, 🌟🎉\n\nYou're a Quiz Champion! 🏆\n\nYou've earned ${rewardCoins} Coins 💰 and ${rewardExp} EXP 🌟\n\nKeep up the great work! 🚀`; event.threadID, event.messageID);
        } else {
